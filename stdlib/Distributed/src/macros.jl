...
# split domain would be more precise
# splitting a right triangle equaly between np processors
function splitdomain(N::Int, np::Int, lower::Bool)
    if N < np || np < 2
        return [1,N]
    end
    result = Array{Real}(uninitialized, 2np)
    denom =  sqrt(np)
    N2 = N*N
    x = 1
    for i = 1:np-1
...
function preducellt(reducer, f, R)
    N = length(R)
    chunks = splitdomain(N, nworkers(), true)
    all_w = workers()[1:length(chunks)]
